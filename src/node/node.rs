...
use crate::params::DIFFICULTY_ADJUSTMENT_INTERVAL;
...
pub struct Node {
    ...
    genesis_accounts: BTreeMap<Address, crate::state::Account>,
}

impl Node {
    pub fn new(ledger: Ledger, storage: Storage, consensus: Consensus) -> Self {
        let genesis_accounts = if ledger.tip_height() == Some(crate::types::Height(0)) {
    ...
    pub fn init_or_load(path: impl AsRef<Path>, consensus: Consensus) -> Result<Self, NodeError> {
        ...
        let genesis_accounts = storage.load_genesis_accounts()?.unwrap_or_else(|| {
            if ledger.tip_height() == Some(crate::types::Height(0)) {
                ledger.accounts.clone()
            } else {
                BTreeMap::new()
            }
    ...
    pub fn available_balance(&self, address: &Address) -> Option<Balance> {
        self.available_balance_with_depth(address, crate::params::FINALITY_DEPTH as u64)
    }
